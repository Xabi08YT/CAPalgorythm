...
def findTuteur(groupLVL,parsedFreetime,parsedSubjects):
    ...
    for i in parsedFreetime:
        for j in parsedSubjects:
            cursor.execute("""SELECT nom, prenom FROM 'tuteur' INNER JOIN 'groupe' ON tuteur.group = group.id WHERE groupe.level >= ? AND freeon LIKE ? AND subject LIKE ?""",(groupLVL,i,j))
